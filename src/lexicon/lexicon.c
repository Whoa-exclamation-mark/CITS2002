    ...
    while (*strings){
        if(*strings[0]!='\t'){
            ...
            if(dependencies!=NULL){
                ...
                char* char_data = calloc(strlen(char_raw), strlen(char_raw));
                space_strip(char_raw,char_data);
                data[i]->name = char_data;

                char_raw = strdup(dependencies+1);
                char_data = calloc(strlen(char_raw), strlen(char_raw));
                space_strip(char_raw,char_data);
                ...
                //we are assuming that all targets have at least 1 command - there is a bug in this!
                strings++;
                ...
                data[i]->file_dependencies = calloc(BUFSIZ*2, BUFSIZ*2);
